...
from .retrieval_models import kpsim_models
...
def ddp_train_model(process_rank, args):
    ...
    # Initialize model.
    if model_name in {'specter'}:
        model = content_profile_models.SPECTER(model_hparams=all_hparams)
    elif model_name in {'kpencconsent'}:
        model = kpsim_models.KeyPhraseAspireWrapper(model_hparams=all_hparams)
    else:
        sys.exit(1)
    ...
    if model_name in ['specter']:
        ...
        batcher_cls = batchers.KPAbsSentTokBatcher
        aspire_config = data_utils.load_aspire_config(
            expanded_model_name=all_hparams['consent-base-pt-layer'])
        batcher_cls.context_bert_config_str = aspire_config['base-pt-layer']
        batcher_cls.kp_bert_config_str = all_hparams['kp-base-pt-layer']
    else:
        sys.exit(1)
        
    if model_name in ['specter', 'kpencconsent']:
        ...
        if model_name in {'kpencconsent'}:
            model_trainer.save_function = trainer.keyphrase_bert_save_function_ddp
        else:
...
def main():
    ...
    train_args.add_argument('--model_name', required=True,
                            choices=['specter', 'cospecter', 'kpencconsent'],
                            help='The name of the model to train.')
    train_args.add_argument('--dataset', required=True,
                            choices=['gorccompscicit', 'citeuliket', 'citeulikea',
                                     'tedrec', 'oriclr2019', 'oriclr2020', 'oruai2019',
                                     'yelppoi'],
