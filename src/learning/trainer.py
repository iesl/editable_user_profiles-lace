...
def keyphrase_bert_save_function_ddp(model, save_path, model_suffix):
    """
    Model saving function used in the training loop.
    This is saving with the assumption that non DDP models will use this model.
    """
    model_fname = os.path.join(save_path, f'kp_encoder_{model_suffix}.pt')
    model = copy.deepcopy(model.module.kp_encoder.state_dict())
    consume_prefix_in_state_dict_if_present(model, "module.")
    torch.save(model, model_fname)
    print('Wrote: {:s}'.format(model_fname))


def keyphrase_bert_save_function(model, save_path, model_suffix):
    """
    Model saving function used in the training loop for sentence bert
    """
    model_fname = os.path.join(save_path, f'kp_encoder_{model_suffix}.pt')
    torch.save(model.kp_encoder.state_dict(), model_fname)
    logging.info('Wrote: {:s}'.format(model_fname))
    

def bertencoder_save_function_ddp(model, save_path, model_suffix):
    """
    Model saving function used in the training loop.
    This is saving with the assumption that non DDP models will use this model.
    """
    model_fname = os.path.join(save_path, f'model_{model_suffix}.pt')
    model = copy.deepcopy(model.module.bert_encoder.state_dict())
    consume_prefix_in_state_dict_if_present(model, "module.")
    torch.save(model, model_fname)
    print('Wrote: {:s}'.format(model_fname))


def bertencoder_save_function(model, save_path, model_suffix):
    """
    Model saving function used in the training loop for sentence bert
    """
    model_fname = os.path.join(save_path, f'model_{model_suffix}.pt')
    torch.save(model.bert_encoder.state_dict(), model_fname)
    logging.info('Wrote: {:s}'.format(model_fname))


class StreamingGenericTrainer:
    ...
    def __init__(self, cml_exp, model, batcher, model_path, train_hparams,
        ...
        self.loss_function_cal = StreamingGenericTrainer.compute_loss
...
class StreamingBasicRankingTrainer(StreamingGenericTrainer):
    def __init__(self, cml_exp, model, batcher, model_path, data_path,
        ...
        """
        StreamingGenericTrainer.__init__(self, cml_exp=cml_exp, model=model, batcher=batcher, model_path=model_path,
        ...
        # Every subclass needs to set this.
        self.loss_function_cal = StreamingBasicRankingTrainer.compute_loss
...
class InMemGenericTrainer:
    # If this isnt set outside of here it crashes with: "got multiple values for argument"
    # todo: Look into who this happens --low-pri.s
    save_function = generic_save_function
    
    def __init__(self, cml_exp, model, batcher, model_path, train_hparams,
                 early_stop=True, verbose=True, epoch_save=False, dev_score='loss'):
        """
        A generic trainer class that defines the training procedure.
        Importantly the data here is all loaded into memory by the batcher rather
        than streamed into the batcher.
        Trainers for other models should subclass this and define the
        data that the models being trained consume.
        :param cml_exp: comet_ml Experiment object.
        :param model: pytorch model.
        :param batcher: a model_utils.Batcher class.
        :param model_path: string; directory to which model should get saved.
        ...
        :param learning_rate: float;
        :param es_check_every: int; check some metric on the dev set every check_every iterations.
            needs a-priori knowledge of the number of batches pick a reasonable value.
        :param lr_decay_method: string; {'exponential', 'warmuplin', 'warmupcosine'}
        :param decay_lr_by: float; decay the learning rate exponentially by the following
            factor.
        :param num_warmup_steps: int; number of steps for which to do warm up.
        ...
        self.train_fnames = {}
        self.dev_fnames = {}
        self.train_batcher = None
        self.dev_batcher = None
        self.num_batches = None
        self.total_train_iters = None
        self.iteration = 0
    ...
    def _init_optimizers(self, train_hparams):
        """
        Code which depends on knowledge of number of batches and should
        be called from subclasses after initializing the batchers.
        """
        # Initialize optimizer.
        if self.update_rule == 'adam':
            self.optimizer = optim.Adam(self.model.parameters(), lr=self.learning_rate)
        ...
        if self.lr_decay_method == 'exponential':
            ...
            self.scheduler = transformers.get_linear_schedule_with_warmup(
                optimizer=self.optimizer, num_warmup_steps=self.num_warmup_steps,
                num_training_steps=self.total_train_iters)
        elif self.lr_decay_method == 'warmupcosine':
            self.num_warmup_steps = train_hparams['num_warmup_steps']
            self.scheduler = transformers.get_cosine_schedule_with_warmup(
                optimizer=self.optimizer, num_warmup_steps=self.num_warmup_steps,
    ...
    def train(self):
        ...
        total_time_per_dev = 0
        train_start = time.time()
        logging.info('Per epoch examples; num_train: {:d}; num_dev: {:d}'.
                     format(self.train_batcher.num_batches*self.batch_size,
                            self.dev_batcher.num_batches*self.batch_size))
        logging.info('Training {:d} epochs, {:d} iterations'.
                     format(self.num_epochs, self.total_train_iters))
        with self.cml_experiment.train():
            for epoch in range(self.num_epochs):
                # Get the next training batch.
                iters_start = time.time()
                for batch_doc_ids, batch_dict in self.train_batcher.next_batch():
                    self.model.train()
                    batch_start = time.time()
                    # Impelemented according to:
                    # https://discuss.pytorch.org/t/why-do-we-need-to-set-the-gradients-
                    ...
                    if self.iteration % self.log_every == 0:
                        ...
                        self.loss_checked_iters.append(self.iteration)
                        if self.verbose:
                            log_str = 'Epoch: {:d}; Iteration: {:d}/{:d}; '.format(epoch, self.iteration, self.total_train_iters)
                            logging.info(log_str + '; '.join(loss_str))
                    elif self.verbose:
                        logging.info('Epoch: {:d}; Iteration: {:d}/{:d}'.
                                     format(epoch, self.iteration, self.total_train_iters))
                    # The decay_lr_every doesnt need to be a multiple of self.log_every
                    if self.iteration > 0 and self.iteration % self.decay_lr_every == 0:
                        self.scheduler.step()
                        # logging.info('Decayed learning rates: {}'.
                    ...
                    with self.cml_experiment.validate():
                        if self.iteration % self.es_check_every == 0 and self.iteration != 0 and self.early_stop:
                            ...
                            if self.dev_score == 'loss':
                                dev_score = -1.0 * pu.inmem_batched_loss(
                                    model=self.model, batcher=self.dev_batcher,
                                    loss_helper=self.loss_function_cal)
                            self.cml_experiment.log_metric(self.dev_score, dev_score, step=self.iteration)
                            dev_end = time.time()
                            total_time_per_dev += dev_end-dev_start
                            ...
                            if dev_score > best_dev_score:
                                ...
                                best_epoch = epoch
                                best_iter = self.iteration
                                everything = (epoch, self.iteration, self.total_train_iters, dev_score)
                                if self.verbose:
                                    logging.info('Current best model; Epoch {:d}; '
                                                 'Iteration {:d}/{:d}; Dev score: {:.4f}'.format(*everything))
                                self.save_function(model=self.model, save_path=self.model_path, model_suffix='cur_best')
                ...
                if self.epoch_save:
                    self.save_function(model=self.model, save_path=self.model_path, model_suffix=f'epoch-{epoch}')
                logging.info('Epoch {:d} time: {:.4f}s'.format(epoch, epoch_time))
                logging.info('\n')
        ...
        if self.total_train_iters > 0:
            self.time_per_batch = float(total_time_per_batch)/self.total_train_iters
        else:
            self.time_per_batch = 0.0
        logging.info('Time per batch: {:.4f}s'.format(self.time_per_batch))
        if self.early_stop and self.dev_score_history:
...
class InMemBasicRankingTrainer(InMemGenericTrainer):
    def __init__(self, cml_exp, model, batcher, model_path, data_path,
                 train_hparams, early_stop=True, verbose=True, epoch_save=False, dev_score='loss'):
        """
        Trainer for any model returning a ranking loss. Uses everything from the
        generic trainer but needs specification of how the loss components
        should be put together.
        :param data_path: string; directory with all the int mapped data.
        """
        InMemGenericTrainer.__init__(self, cml_exp=cml_exp, model=model, batcher=batcher, model_path=model_path,
                                     train_hparams=train_hparams, early_stop=early_stop, verbose=verbose,
                                     epoch_save=epoch_save, dev_score=dev_score)
        # The default is a cold start setup. But this will be set to true when its a warm start.
        warm_start = train_hparams.get('warm_start', False)
        if warm_start:
            cold_warm_start = 'warm_start'
        else:
            simpair = train_hparams.get('simpair', False)  # For the editability eval.
            if simpair:
        ...
        if 'train_suffix' in train_hparams:
            suffix = train_hparams['train_suffix']
            self.train_fnames = {
                'docs_fname': os.path.join(data_path, cold_warm_start, f'train-abstracts-{suffix}.pickle'),
                'uid2anns_fname': os.path.join(data_path, cold_warm_start, f'train-uid2anns.pickle')
            }
            self.dev_fnames = {
                'docs_fname': os.path.join(data_path, cold_warm_start, f'dev-abstracts-{suffix}.pickle'),
                'uid2anns_fname': os.path.join(data_path, cold_warm_start, f'dev-uid2anns.pickle')
            }
        else:
            self.train_fnames = {
                'docs_fname': os.path.join(data_path, cold_warm_start, f'train-abstracts.pickle'),
                'uid2anns_fname': os.path.join(data_path, cold_warm_start, f'train-uid2anns.pickle')
            }
            self.dev_fnames = {
                'docs_fname': os.path.join(data_path, cold_warm_start, f'dev-abstracts.pickle'),
                'uid2anns_fname': os.path.join(data_path, cold_warm_start, f'dev-uid2anns.pickle')
            }
        
        # Initialize the batchers.
        self.train_batcher = batcher(ex_fnames=self.train_fnames, all_hparams=train_hparams)
        self.dev_batcher = batcher(ex_fnames=self.dev_fnames, all_hparams=train_hparams)
        self.num_batches = self.train_batcher.num_batches
        self.total_train_iters = self.num_epochs * self.num_batches
        
        # Initialize the optimizers once the batches have been computed.
        self._init_optimizers(train_hparams=train_hparams)
        
        # Every subclass needs to set this.
        self.loss_function_cal = InMemBasicRankingTrainer.compute_loss
...
class InMemGenericTrainerSepLosses:
    # If this isnt set outside of here it crashes with: "got multiple values for argument"
    # todo: Look into who this happens --low-pri.s
    save_function = generic_save_function
    ...
    def __init__(self, cml_exp, model, batcher, model_path, train_hparams,
        ...
        """
        A generic trainer class that defines the training procedure.
        - The data here is all loaded into memory by the batcher rather
        than streamed into the batcher.
        - The model is also expected to return two separate losses and different parts of the model
        are trained from each in the training loop by maintaining two optimizers.
        Trainers for other models should subclass this and define the
        data that the models being trained consume.
        :param cml_exp: comet_ml Experiment object.
        :param model: pytorch model.
        ...
        # Every subclass needs to set this.
        self.dev_loss_function_cal = None
    ...
    def _init_optimizers(self, train_hparams):
        ...
        be called from subclasses after initializing the batchers.
        """
        # Initialize optimizer; the main recommendation loss trains the sentence encoder
        main_params = []
        for name, param in self.model.named_parameters():
            if str.startswith(name, 'sent_encoder'):
                main_params.append(param)
        self.optimizer = optim.Adam(main_params, lr=self.learning_rate)
        # The aux ict loss trains the kp encoder.
        aux_params = []
        for name, param in self.model.named_parameters():
            if str.startswith(name, 'kp_encoder'):
                aux_params.append(param)
        self.optimizer_aux = optim.Adam(aux_params, lr=self.learning_rate)
        # Reduce the learning rate every few iterations.
        self.lr_decay_method = train_hparams['lr_decay_method']
        self.decay_lr_every = train_hparams['decay_lr_every']
        self.log_every = 5
        ...
        self.scheduler = transformers.get_linear_schedule_with_warmup(
            optimizer=self.optimizer, num_warmup_steps=self.num_warmup_steps,
            num_training_steps=self.total_train_iters)
        self.scheduler_aux = transformers.get_linear_schedule_with_warmup(
            optimizer=self.optimizer_aux, num_warmup_steps=self.num_warmup_steps,
            num_training_steps=self.total_train_iters)
            ...
            for epoch in range(self.num_epochs):
                ...
                for batch_doc_ids, batch_dict in self.train_batcher.next_batch():
                    ...
                    if self.accumulate_gradients:
                        # Compute objective.
                        ret_dict = self.model.forward(batch_dict=batch_dict)
                        objective, objective_aux = self.compute_loss(loss_components=ret_dict)
                        # Gradients wrt the parameters.
                        objective.backward(retain_graph=True)
                        objective_aux.backward()
                        if (self.iteration + 1) % self.update_params_every == 0:
                            self.optimizer.step()
                            self.optimizer.zero_grad()
                    ...
                    with self.cml_experiment.validate():
                        if self.iteration % self.es_check_every == 0 and self.iteration != 0 and self.early_stop:
                            ...
                            if self.dev_score == 'loss':
                                dev_score = -1.0 * pu.inmem_batched_loss(
                                    model=self.model, batcher=self.dev_batcher,
                                    loss_helper=self.dev_loss_function_cal)
                            self.cml_experiment.log_metric(self.dev_score, dev_score, step=self.iteration)
                            dev_end = time.time()
                            total_time_per_dev += dev_end-dev_start
                            self.dev_score_history.append(dev_score)
...
class InMemBasicRankingTrainerSepLosses(InMemGenericTrainerSepLosses):
    def __init__(self, cml_exp, model, batcher, model_path, data_path,
                 train_hparams, early_stop=True, verbose=True, dev_score='loss'):
        """
        Trainer for any model returning a ranking loss. Uses everything from the
        ...
        :param data_path: string; directory with all the int mapped data.
        """
        InMemGenericTrainerSepLosses.__init__(self, cml_exp=cml_exp, model=model, batcher=batcher,
                                              model_path=model_path, train_hparams=train_hparams,
                                              early_stop=early_stop, verbose=verbose, dev_score=dev_score)
        # The default is a cold start setup. But this will be set to true when its a warm start.
        warm_start = train_hparams.get('warm_start', False)
        if warm_start:
            cold_warm_start = 'warm_start'
        ...
        # Every subclass needs to set this; this is the function used for helping compute the loss in dev time
        self.dev_loss_function_cal = InMemBasicRankingTrainerSepLosses.compute_dev_loss
    
    @staticmethod
    def compute_loss(loss_components):
        """
        Return both losses - used at train time.
        :param loss_components: dict('rankl': rank loss value)
        :return: Variable.
        """
        return loss_components['rankl'], loss_components['aux_rankl']

    @staticmethod
    def compute_dev_loss(loss_components):
        """
