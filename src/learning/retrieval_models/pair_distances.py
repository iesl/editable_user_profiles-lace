...
def allpair_masked_dist_l2lse(query, cand, return_pair_sims=False):
    """
    ...
    # Changed this to a max because logsumexp with negative distances is a bit weird.
    # renaming function and such is for later. Shrug.
    batch_sims, indices = torch.max(pair_sims.view(qef_batch_size, qmax_sents * cmax_sents), dim=1)
...
def allpair_masked_dist_l2sum(query, cand, return_pair_sims=False):
    """
    Given a set of query and candidate reps compute L2 similarity
    between all the query reps and all the candidate reps,
    then sum the similarities.
    :param query: namedtuple(
        embed: batch_size x encoding_dim x q_max_sents;
        abs_lens: list(int); number of sentences in every batch element.)
    :param cand: namedtuple(
    ...
    # (effective) batch_size x qmax_sents x cmax_sents
    # inputs are: batch_size x encoding_dim x c/qmax_sents so permute them.
    neg_pair_dists = torch.cdist(query_reps.permute(0, 2, 1), cand_reps.permute(0, 2, 1))
    if len(neg_pair_dists.size()) == 2:
        neg_pair_dists = neg_pair_dists.unsqueeze(0)
    assert (neg_pair_dists.size(1) == qmax_sents)
    assert (neg_pair_dists.size(2) == cmax_sents)
    ...
    # Negation of L2 distance isnt a similarity.
    if return_pair_sims:
        pair_sims = 1.0 / (neg_pair_dists + 1.0)
        batch_sims = torch.sum(neg_pair_dists.view(qef_batch_size, qmax_sents * cmax_sents), dim=1)
        return batch_sims, pair_sims
    # Return a distance instead of a similarity - so the distances are minimized even further.
    else:
        # Simply sum the scores; pad positions will be zeros.
        batch_sims = torch.sum(neg_pair_dists.view(qef_batch_size, qmax_sents * cmax_sents), dim=1)
        return batch_sims


def allpair_masked_dist_l2maxsum(query, cand, return_pair_sims=False):
    """
    Given a set of query and candidate reps compute l2 distances
    between all the query facet reps and all the candidate facet reps,
    then aggregate similarity with summation of max sims of the query reps
    to the cand reps.
    :param query: namedtuple(
        embed: batch_size x encoding_dim x q_max_sents;
        abs_lens: list(int); number of sentences in every batch element.)
    ...
    # Add very large negative values in the pad positions which will be zero.
    neg_pair_dists = neg_pair_dists + pad_mask
    # Max across the candidate reps.
    # - because these are negative distances the smallest distances will be picked.
    batch_sent_dists, indices = torch.max(neg_pair_dists, dim=2)
    # Return a distance instead of a similarity - so the smallest distances are minimized even further.
    if return_pair_sims:
        # L2 distance to similarity: https://stats.stackexchange.com/a/53070/55807
        sent_sims = 1.0 / ((-1 * batch_sent_dists) + 1.0)
        batch_sims = torch.sum(sent_sims, dim=1)
        pair_sims = 1.0 / ((-1 * neg_pair_dists) + 1.0)
        return batch_sims, pair_sims
    else:
        batch_sims = torch.sum(batch_sent_dists, dim=1)
        return -1 * batch_sims